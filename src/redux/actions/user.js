...
export const register = (email,password,name,username) => async (dispatch) => {
    try {
        ...
        const isAuthenticate = await createUserWithEmailAndPassword(FIREBASE_AUTH,email,password);
        const user = FIREBASE_AUTH.currentUser
...
export const saveUserProfileImage =  (avatar) => async (dispatch) => {
    try {
        const url = await saveMediaToStorage(avatar, `user/${FIREBASE_AUTH.currentUser.uid}/avatar`);
        const docRefUser = doc(collection(FIREBASE_DB, 'user'), FIREBASE_AUTH.currentUser.uid);
...
export const saveUserField =  (name,value) => async (dispatch) => {
    try {
        ...
        if(name == 'username'){
            const docRefUsername = doc(collection(FIREBASE_DB, 'username'), FIREBASE_AUTH.currentUser.uid);
...
export const getUsername = () => async (dispatch) => {
    try {
        const unsubscribe = onSnapshot(collection(FIREBASE_DB, 'username'), (snapshot) => {
            const username = [];
            snapshot.forEach((doc) => {
                const data = doc.data();
            ...
            if(docSnapshotLike.exists()){
                ...
                if(dataLike.users.includes(FIREBASE_AUTH.currentUser.uid)){
                    ...
                    await updateDoc(docRefLike,{
                        users: [...dataLike.users.filter(uid => uid != FIREBASE_AUTH.currentUser.uid)]
                    })
                }else{
                    await updateDoc(docRefPost,{
                        likesCount: dataLike.users.length+1
...
export const followUser = async (userId, isFollow) => {
    try {
        ...
        const docRefUserFollowing = doc(collection(FIREBASE_DB, 'user'), FIREBASE_AUTH.currentUser.uid);
        const docRefFollowing = doc(collection(FIREBASE_DB, 'following'), FIREBASE_AUTH.currentUser.uid);
        ...
        if (docSnapshotFollow.exists()) {
            ...
            if (isFollow) {
                ...
                await updateDoc(docRefFollow, {
                    users: followData.users.filter(uid => uid !== FIREBASE_AUTH.currentUser.uid)
                });
                ...
                if (!docSnapshotFollowing.exists()) {
                    ...
                    await updateDoc(docRefFollowing, {
                        users: docSnapshotFollowing.data().users.filter(uid => uid !== userId)
                    });
                }
